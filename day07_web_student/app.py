...
def segments():
    ...
    segment_rows = segment_df[["TenureGroup", "用户数", "流失人数", "流失率"]].copy()
    segment_rows["流失率"] = segment_rows["流失率"].map(lambda value: f"{value:.1%}")
    return render_template(
        "segments.html",
        username=session["username"],
        segment_rows=segment_rows.to_dict("records"),
    )
